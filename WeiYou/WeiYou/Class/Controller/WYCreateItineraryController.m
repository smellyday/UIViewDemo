...
@implementation WYCreateItineraryController {
...
- (void)onClickOK:(id)sender {
    if (_tripNameField.text != nil && [_tripNameField.text length] != 0) {
        WYUserTrip *mTrip = [[WYUserTrip alloc] initUserTripWithName:_tripNameField.text];
        [[WYUserTripAgent sharedUserTripAgent] addNewUserTrip:mTrip];
        
        [self dismissKeyboard:sender];
        [self dismissViewControllerAnimated:YES completion:nil];
    } else {
        UIAlertView *mAlert = [[UIAlertView alloc] initWithTitle:@"name can not be empty" message:nil delegate:nil cancelButtonTitle:@"Cancel" otherButtonTitles:@"OK", nil];
        [mAlert show];
    }
